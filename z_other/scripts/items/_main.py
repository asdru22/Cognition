...
def get_item_model(id, data):
    if "type" in data:
        return f"{namespace}:{data['type']}/{id}"
    else:
...
def get_lore(data):
    r = {"function": "minecraft:set_lore",
         "lore": [{"translate": f"pack.{namespace}", "color": "#653AA0", "italic": False}], "mode": "replace_all"}
    if "lore" in data:
        r["lore"].append("")
...
def count_loot_table(type, id, count):
    return json.dumps({"pools": [
        {"rolls": 1, "entries": [{"type": "minecraft:loot_table", "value": f"cgn:{type}/{id}"}],
        ...
        if not row == []:
            ...
            for item in row:
                if item == "":
                    ...
                    out += "{Slot:" + str(
                        slot) + "b,components:{\"minecraft:item_name\":'{\"translate\":\"item.cgn." + item[
                                                                                                      4:] + "\"}'}}"
                slot += 1
                if not slot == 3:
                    out += ","
...
def base_loot_table(base_item, id, data):
    return {"pools": [{"rolls": 1, "entries": [{"type": "minecraft:item", "name": f"minecraft:{base_item}"}],
                       "functions": [{"function": "minecraft:set_components",
                                      "components": {"minecraft:tooltip_style": f"{namespace}:default",
                                                     "minecraft:item_model": get_item_model(id=id, data=data)}}]}]}

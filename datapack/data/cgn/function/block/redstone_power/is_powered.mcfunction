...
#generic: static
execute if block ~ ~1 ~ #cgn:strong_power_top unless block ~ ~1 ~ #cgn:strong_power_top[power=0] unless block ~ ~1 ~ #cgn:strong_power_top[powered=false] run scoreboard players set #out cgn.dummy 1
execute if block ~ ~-1 ~ #cgn:strong_power_bottom[powered=true,face=ceiling] run scoreboard players set #out cgn.dummy 1
execute if block ~1 ~ ~ #cgn:strong_power_side[powered=true,facing=east] run scoreboard players set #out cgn.dummy 1
execute if block ~-1 ~ ~ #cgn:strong_power_side[powered=true,facing=west] run scoreboard players set #out cgn.dummy 1
execute if block ~ ~ ~1 #cgn:strong_power_side[powered=true,facing=south] run scoreboard players set #out cgn.dummy 1
execute if block ~ ~ ~-1 #cgn:strong_power_side[powered=true,facing=north] run scoreboard players set #out cgn.dummy 1

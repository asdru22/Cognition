...
execute if entity @s[predicate=cgn:entity_properties/double_edged_sword_projectile/break] run function cgn:item/double_edged_sword/drop/item

execute if entity @s[predicate=cgn:entity_properties/double_edged_sword_projectile/go_to_player] anchored eyes run function cgn:item/double_edged_sword/tick/track_owner with entity @s item.components."minecraft:custom_data".cgn

scoreboard players operation #temp cgn.dummy = @s cgn.id
scoreboard players reset #kill cgn.dummy

execute as @e[type=!#cgn:inanimate,predicate=cgn:entity_properties/double_edged_sword_projectile/can_hit,distance=..10,dx=0] positioned ~-0.6 ~-0.8 ~-0.6 if entity @s[dx=0] run function cgn:item/double_edged_sword/hit/main

execute if score @s cgn.dummy matches 5.. as @a[gamemode=!spectator,distance=..5,dx=0] positioned ~-0.6 ~-0.6 ~-0.6 if entity @s[dx=0] positioned ~0.6 ~0.6 ~0.6 if score @s cgn.id = #temp cgn.dummy run function cgn:item/double_edged_sword/drop/find_owner
